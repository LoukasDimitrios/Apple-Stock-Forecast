...
# --- Configuration ---
SEQ_LENGTH = 20
features = ['Lag_7', 'Rolling_Return_5', 'Lag_8', 'Lag_6', 'RSI_14', 'DayOfWeek']
target_col = 'Target_3d'

# --- Load data ---
df = pd.read_csv("data/apple_stock_data_enriched.csv", index_col="Date", parse_dates=True)

# Compute 3-day forward return and binary target
...
# Save scaler
joblib.dump(scaler, "models/scaler_lstm_best_grid_target3d_seq20_feats6.pkl")
...
# --- Save Results ---
torch.save(model.state_dict(), "models/lstm_best_grid_target3d_seq20_feats_6_thresh05.pt")

probs_array = np.array(probs_all).flatten()
np.save("results/y_true_lstm_shap.npy", y_true)
# np.save("results/preds_lstm_shap.npy", preds)
np.save("results/probs_lstm_shap.npy", probs_array)
...
metrics = {
    ...
    "AUC": roc_auc_score(y_true, probs_array)
}
pd.DataFrame([metrics]).to_csv("results/lstm_shap_results.csv", index=False)
